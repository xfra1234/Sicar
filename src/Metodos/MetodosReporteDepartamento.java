...
////es el mismo solamente acomodado para que funcione en el servidor de bodega, en la caja de ventas a domicilio
    public void excelbodega(String fecha1, String fecha2, String fechauno, String fechados, int sucursal) {
        try (FileInputStream file = new FileInputStream(new File(abrirarchivo))) {
            // leer archivo excel
            POIFSFileSystem fs = new POIFSFileSystem(file);
            HSSFWorkbook libro = new HSSFWorkbook(fs);
...
///////////     fin
    ...
    public void excelsucursalbodega(String fecha1, String fecha2, String fechauno, String fechados, int sucursal) {
        try (FileInputStream file = new FileInputStream(new File(abrirarchivo))) {
            // leer archivo excel
            POIFSFileSystem fs = new POIFSFileSystem(file);
            HSSFWorkbook libro = new HSSFWorkbook(fs);
...
//////////////////// inicio ventas por notas de venta
            ...
            try {
                ...
                rs = stmt.executeQuery("select MONTHNAME(venta.fecha) mes ,sum(detallev.importecon) as suma "
                        ...
                        + " and venta.fecha >= date_sub('" + fecha1 + "', interval 0 month)"
                        + " and venta.fecha <= date_sub('" + fecha2 + "', interval 0 month)"
                        + "venta.caj_id=3 departamento.dep_id = 22 and venta.status!= -1 and venta.tic_id is  null "
                        + " and venta.fecha >= date_sub('" + fecha1 + "', interval 0 month)"
                        + " and venta.fecha <= date_sub('" + fecha2 + "', interval 0 month)  group by month(fecha )  "
                        + "order by  year(fecha), month(fecha) ;");
                while (rs.next()) {
